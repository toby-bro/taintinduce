...
def x86_instruction_data(x86_state_format):
    ...
    Usage in tests:
        def test_something(x86_instruction_data):
    ...
    class LazyCache:
        ...
        def __getitem__(self, key: str) -> dict[str, Any]:
            if key not in self._cache:
                ...
                bytestring = key
                obs_engine = ObservationEngine(bytestring, 'X86', x86_state_format)
...
def test_and_eax_ebx_observation_generation(x86_instruction_data):
...
def test_and_eax_ebx_full_inference(x86_instruction_data):
...
def test_and_eax_ebx_rule_explains_observations(x86_instruction_data):
...
def test_and_eax_ebx_captures_all_flows(x86_instruction_data):
...
def test_and_eax_ebx_ebx_always_unconditional(x86_instruction_data):
    ...
    for i in range(32):  # 32 bits in EBX
        ...
        for pair in rule.pairs:
            if pair.condition is None or pair.condition.condition_ops is None:
                # Unconditional pair
                if ebx_bit in pair.output_bits and ebx_bit in pair.output_bits[ebx_bit]:
...
def test_and_eax_ebx_conditional_propagation(x86_instruction_data):
    ...
    for i in test_bits:
        ...
        for pair in rule.pairs:
            ...
            if ebx_bit in pair.output_bits and eax_bit in pair.output_bits[ebx_bit]:
...
def test_and_eax_ebx_eax_identity_propagation(x86_instruction_data):
    ...
    for i in test_bits:
        ...
        found = False
        for pair in rule.pairs:
            if eax_bit in pair.output_bits and eax_bit in pair.output_bits[eax_bit]:
...
def test_and_eax_ebx_condition_only_uses_relevant_bits(x86_instruction_data):
    ...
    for pair in rule.pairs:
        if eax_bit_1 in pair.output_bits and eax_bit_1 in pair.output_bits[eax_bit_1]:
...
def test_and_eax_ebx_check_which_inputs_affect_eax1(x86_instruction_data):
